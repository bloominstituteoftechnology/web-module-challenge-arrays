...
function copy(originalFlavors, updatedFlavors) {
    updatedFlavors = [...originalFlavors];
...
function filterByWord(array, flavorZ){
    let filteredArray = [];
    for (i = 0; i < array.length; i++) {
        if (array[i].includes(flavorZ)) {
...
function getAverageWordLength(originalFlavors){
  ...
  for (i = 0; i < originalFlavors.length; i++) {
    wordCount[i] = originalFlavors[i].split(" ").length;
  }
