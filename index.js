...
function is31Flavors(arrayOne){
   if (arrayOne.length == 31) {
       return true;
   }
   else {
       return false;
...
function addFlavor(arrayOne, flavorString){
    arrayOne.unshift(flavorString);
    return arrayOne;
}
...
function getFlavorByIndex(arrayOne, number){
...
function removeFlavorByName(arrayOne, flavorName){
    arrayOne.splice(arrayOne.indexOf(flavorName), 1);
    return arrayOne;
...
function filterByWord(arrayOne, flavorName){
    const result = arrayOne.filter(flavor => flavor.includes(flavorName));
    return result;
}
