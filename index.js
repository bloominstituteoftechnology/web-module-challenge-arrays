...
function removeFlavorByName(originalFlavors, remove){
    var start = originalFlavors.indexOf(remove)
     originalFlavors.splice(start, 1)
...
function filterByWord(originalFlavors, search){
    let filtered = [];
     for (var j = 0; j < originalFlavors.length; j++){
       if (originalFlavors[j].includes(search)){
         filtered.push(originalFlavors[j])
       }
     }
     return filtered
  }
