...
function addFlavor(anArray, newFlavors){
   anArray.unshift(newFlavors);
   return anArray;
}
...
function getFlavorByIndex(anArray, aFlavor){
    return anArray[aFlavor];
}
...
function removeFlavorByName(anArray, aFlavor){
    for(let i=0; i < anArray.length; i++)
    {if (anArray[i] === aFlavor){anArray.splice(i,1)}}
    return anArray;
}
...
function filterByWord(anArray, aFlavor){
    const holidayFlavor = [];
    for (let i=0; i < anArray.length; i++){
        if (anArray[i].includes(aFlavor)){
        holidayFlavor.push(anArray[i]);}
    }
