...
function addFlavor (icecream, newFlavor){
   return icecream.unshift(newFlavor);  
} 
...
*/
  ...
  function removeLastFlavor(icecream){
      icecream.pop();
      return icecream;
    }
...
*/
  ...
  function getFlavorByIndex(icecream, index){
   return icecream[index];
    } 
    getFlavorByIndex(noriginalFlavors, 7);
...
function removeFlavorByName(icecream, removedFlavor){
    icecream.splice(icecream.indexOf(removedFlavor),1);
    return icecream;
}
removeFlavorByName(newFlavors, "Rocky Road");
...
function filterByWord(icecream, removedFlavor){
    let selectedFlavors = [];
    for(let item of icecream){
        if (item.includes(removedFlavor)){
