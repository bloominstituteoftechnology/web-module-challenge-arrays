...
const originalFlavorCopy = copy(originalFlavors); 
...
is31Flavors(originalFlavorCopy);
...
addFlavor(originalFlavorCopy, "Rainbow Sherbert");
...
removeLastFlavor(originalFlavorCopy);
...
function getFlavorByIndex(arrayName, indexNumber){
    return arrayName[indexNumber];
}

getFlavorByIndex(originalFlavorCopy, 2);
...
function removeFlavorByName(arrayName, flavorName){
    const copiedArray = [...arrayName];
    for(let i = 0; i < copiedArray.length; i++){
        if(copiedArray[i] === flavorName){
...
const removedFlavor = removeFlavorByName(originalFlavorCopy, "Rocky Road");
...
function filterByWord(arrayName, flavorName){
        const copiedArray = [...arrayName];
        const filteredArray = [];
        for(let i = 0; i < copiedArray.length; i++){
            if(copiedArray[i].includes(flavorName)){
                filteredArray.push(copiedArray[i]);
        }
...
filterByWord(originalFlavorCopy, "chocolate");
...
function getAverageWordLength(arrayName){
    
    const wordCount = [];
    for(let i = 0; i < arrayName.length; i++){
        if(arrayName[i].includes(" ")){
          const newWordCount = [];
          wordCount.push(arrayName[i].split(" ").length);
    }
        else(wordCount.push(1))
...
function getAverageWordLengthPart2(array) {
let x = 0;
...
const arrayCount = getAverageWordLength(originalFlavors)

console.log(getAverageWordLengthPart2(arrayCount))
