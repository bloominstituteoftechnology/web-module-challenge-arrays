...
function removeFlavorByName(arr, flavorToRemove){
    let flavorIndex = arr.indexOf(flavorToRemove);
...
function filterByWord(arr, specifiedFlavor){
  let specialFlavors = [];
  for (let i = 0; i < arr.length; i++) {
    if (arr[i].includes(specifiedFlavor)) {
      specialFlavors.push(arr[i]);
    }
  }
  return specialFlavors;
}
