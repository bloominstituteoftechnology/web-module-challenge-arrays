...
function addFlavor(RainbowSherbert, originalFlavors){
...
originalFlavors.splice(originalFlavors.length -1,1);
}
...
function removeFlavorByName(flavor, originalFlavors){
var index = originalFlavors.indexOf (flavor);
    
    return originalFlavors.splice(index,1);
}
...
function copy(originalFlavors, copyFlavors){
    copyFlavors = originalFlavors.slice();
...
function filterByWord(originalFlavors, searchTerm) {

    let foundFlav = [];
      for (var i = 0; i <  originalFlavors.length; i++){
    
            if (originalFlavors[i].includes(searchTerm))  {
                 foundFlav.push(originalFlavors[i]);
            }
        }
        return foundFlav;
