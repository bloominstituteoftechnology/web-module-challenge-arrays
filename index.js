...
function getAverageWordLength(array){
  ...
  let average = words / array.length;
  return average;
}
...
function getRandomFlavors(array1, array2, array3, array4){
  const random = [...array1, ...array2, ...array3, ...array4];
  function randomFlavors(array, number){
    let picks = [];
    for(let i = 0; i < number; i++){
      picks.push(array[Math.floor(Math.random()*array.length)])
    }
    return picks;
    }
  return randomFlavors(random, 31)
}
