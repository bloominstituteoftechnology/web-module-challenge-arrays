...
function addFlavor(originalFlavors, newFlavor){
  ...
  console.log(addFlavor(originalFlavors, "Peanut Butter"))
...
function removeFlavorByName(originalFlavors, rFlavor){
    let r = (rFlavor)
    originalFlavors.splice(r, 1);
...
function filterByWord(originalFlavors, promo){
    let word = []
    for (let c = 0; c < originalFlavors.length; c++) {
      if (originalFlavors[c].includes(promo)) {
        word.unshift(originalFlavors[c])
      }
    }
    return word
  }
