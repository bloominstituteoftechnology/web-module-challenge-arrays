...
let originalFlavorsCopy = copy(originalFlavors);
...
let newFlavorList = addFlavor(originalFlavorsCopy, "Rainbow Sherbert");
...
function getFlavorByIndex(list, indexNumber){
    return list[indexNumber];
}
// let flavorAt1= getFlavorByIndex(originalFlavor, 30);
...
function removeFlavorByName(list, flavorName){
   for (let i=0; list.length; i++) {
       if (flavorNames= list[i]) {
           list.splice(i, 1)
       }
   }
   return list;
}
let withRemovedFlavors= removeFlavorByName(originalFlavorCopy, "Rocky Road");
...
function filterByWord(list, flavorType){
    let newList = [];
    for (let i = 0; i< list.length - 1; i++){
        if (list[i].includes(flavorType)) {
            newList.push(list[i]);
        }
    }
    return newList;
