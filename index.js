...
function addFlavor(iceCream, newTopping){
   var newTop = originalFlavors.shift(newTop);
   return newTop
}
...
function removeLastFlavor(something){
   var newtop = orignalFlavors.pop();
   return something;
}
...
function getFlavorByIndex(originalFlavors, indexPos){
    return originalFlavors[indexPos, 3]
}
...
function removeFlavorByName(originalFlavors, flavor){
    var  newArray = delete originalFlavors(flavor);
    return newArray;
}
...
function filterByWord(oldList,flavor){
    var newArray = []
    originalFlavors.forEach(flavor)
    function testchoco(x){
        if (x.includes(flavor)){
            newArray.push(x)
        }
    }
    return items;
