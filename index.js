...
let flavorsLength = originalFlavors.length;

function is31Flavors(arr) {
    return flavorsLength === 31;
}
...
function getFlavorByIndex(arr, itemNumber){
...
function removeFlavorByName(arr, goodbyeFlavor) {
    let flavorIndex = arr.indexOf(goodbyeFlavor);
...
function filterByWord(arr, filterWord) {
    const filteredFlavors = [];

    for(let i = 0; i < arr.length; i++) {
        if(arr[i].includes(filterWord)) {
