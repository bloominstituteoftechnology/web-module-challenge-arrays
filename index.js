...
let shiftedFlavors = originalFlavors.shift( 2, 5 );
...
if is31Flavors(originalFlavors[originalFlavors.legth 31]);
{
...
let addedFlavor(originalFlavors.push('Watermelon'));
...
let removeLastFlavor(originalFlavors.pop(originalFlavors.length 31 );
...
*/
    ...
    let getFlavorByIndex(originalFlavors, 2)
{
        return InputDeviceInfo(originalFlavors.unshift('Black Walnut'))
}
...
var removeFlavorByName(originalFlavors.splice( "r", "R" ));
console.log(removeLastFlavor);
...
let filterByWord = [];
for ( let x = 0; x < originalFlavors.length; x++)
  {
    if ( originalFlavors[x].slice(0, 1).toUpperCase() === 'C')
      {
        filterByWord.push( originalFlavors[x] );
      }
...
let getAverageWordLength = [];
for ( let x = 0; x < originalFlavors.length; x++)
  {
    if ( originalFlavors[x].slice(0, 1).length === ( 0 , 3))
      {
        getAverageWordLength.push( originalFlavors[x] );
      }
    

  }

log(getAverageWordLength);
